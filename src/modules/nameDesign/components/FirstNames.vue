...
import { DATA_Chases, DATA_Genders } from "../../../enums/character.enum";
  ...
  <template v-for="chase of chases" :key="chase">
    <template v-for="gender of genders" :key="gender">
      <first-name v-if="filterNames[defineDataKey({ chase, gender })].length" :chase="chase" :gender="gender"
        :first-names="filterNames[defineDataKey({ chase, gender })]">
        / {{ DATA_Chases[chase].translator.title }} / {{ DATA_Genders[gender].title }}
      </first-name>
    </template>
  </template>
  <template v-for="chase of chases" :key="chase">
    <first-name :chase="chase" :first-names="filterNames[defineDataKey({ chase })]">
      / {{ DATA_Chases[chase].translator.title }}
    </first-name>
  </template>
  <template v-for="gender of genders" :key="gender">
    <first-name :gender="gender" :first-names="filterNames[defineDataKey({ gender })]">
      / {{ DATA_Genders[gender].title }}
    </first-name>
  </template>
